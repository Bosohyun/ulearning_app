...
class CourseDetailController {
    ...
    if (result.code == 200) {
      if (context.mounted) {
        context.read<CourseDetailBloc>().add(
              TriggerCourseDetail(courseItem: result.data!),
            );
      } else {}
    } else {
  ...
  asyncLoadLessonData(int? id) async {
    ...
    print('----------my lesson data ${result.data.toString()}------');

    if (result.code == 200) {
      if (context.mounted) {
        context.read<CourseDetailBloc>().add(
              TriggerLessonList(lessonItem: result.data!),
            );
      } else {}
    } else {
      toastInfo(msg: "Something went wrong");
  ...
  void goBuy(int? id) async {
    ...
    if (result.code == 200) {
      ...
      var res = await Navigator.of(context)
          .pushNamed(AppRoutes.PAY_WEB_VIEW, arguments: {
        'url': url,
      });
